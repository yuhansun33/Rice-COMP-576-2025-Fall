...
class NeuralNetwork(object):
    ...
    def backprop(self, X, y):
        ...
        num_examples = len(X)
        delta3 = self.probs # dL/dz2
        delta3[range(num_examples), y] -= 1
        
        # dw2 = dL/dz2 * dz2/dw2 = delta3 * a1
        dW2 = (self.a1.T).dot(delta3)
        # db2 = dL/dz2 * dz2/db2 = delta3
        db2 = np.sum(delta3, axis=0, keepdims=True)
        
        # hidden layer
        # dL/dz2 * dz2/da1 * da1/dz1 = delta3 * W2 * diff_actFun(z1)
        delta2 = delta3.dot(self.W2.T) * self.diff_actFun(self.z1, self.actFun_type)
        
        # dw1 = dL/dz1 * dz1/dw1 = delta2 * X
        dW1 = np.dot(X.T, delta2)
        # db1 = dL/dz1 * dz1/db1 = delta2
